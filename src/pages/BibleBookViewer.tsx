...
export default function BibleBookViewer() {
  ...
  return (
    <div className="h-[100dvh] min-h-screen min-h-[100dvh] flex flex-col bg-white overflow-x-hidden w-full max-w-full">
      ...
      <button
        onClick={scrollToTop}
        className="fixed right-4 xs:right-5 bottom-[max(3.6rem,calc(env(safe-area-inset-bottom)+3.25rem))] z-40 w-11 h-11 rounded-full bg-[#1B64F2] text-white shadow-lg hover:bg-[#1557e0] active:opacity-90 touch-target"
        aria-label="맨 위로"
        title="맨 위로"
      >
        ↑
